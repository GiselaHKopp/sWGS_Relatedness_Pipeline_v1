...
```bash
nextflow run nf-core/swgsrelate --input ./samplesheet.csv --outdir ./results  -profile docker
```
...
```bash
nextflow run nf-core/swgsrelate -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/swgsrelate
```
...
First, go to the [nf-core/swgsrelate releases page](https://github.com/nf-core/swgsrelate/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
